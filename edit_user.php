...
$page_title = getPageTitle(EDIT_USER_TITLE);
...
if (isset($_POST['update'])) {
    ...
    if (empty($errors)) {
        ...
        if ($result && $db->affected_rows() === 1) {
            $session->msg('s', USER.UPDATED_SUCCESSFULLY.'!');
...
if (isset($_POST['update-pass'])) {
    ...
    if (empty($errors)) {
        ...
        $result = $db->query($sql);
        if ($result && $db->affected_rows() === 1) {
            $session->msg('s', USER_PASSWORD_HAS_BEEN_UPDATED.'!');
            redirect('edit_user.php?id=' . (int) $e_user['id'], false);
        } else {
            $session->msg('d', SORRY.FAILED_TO_UPDATED.USER_PASSWORD.'!');
            redirect('edit_user.php?id=' . (int) $e_user['id'], false);
        }
    } else {
        $session->msg("d", $errors);
        ...
        <div class="panel panel-default">
            <div class="panel-heading">
                <strong>
                    ...
                    <? echo UPDATE ?> <?php echo remove_junk(ucwords($e_user['name'])); ?> <? echo ACCOUNT ?>
                </strong>
            </div>
                ...
                <form method="post" action="edit_user.php?id=<?php echo (int) $e_user['id']; ?>" class="clearfix">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" name="status">
                            <option <?php if ($e_user['status'] === '1') echo 'selected="selected"'; ?>value="1"><?php echo ACTIVE; ?></option>
                            <option <?php if ($e_user['status'] === '0') echo 'selected="selected"'; ?> value="0"><?php echo INACTIVE; ?></option>
                        </select>
                    </div>
    ...
    <div class="col-md-6">
        <div class="panel panel-default">
            <div class="panel-heading">
                <strong>
                    <span class="glyphicon glyphicon-th"></span>
                    <? echo CHANGE ?> <?php echo remove_junk(ucwords($e_user['name'])); ?> <? echo PASSWORD ?>
                </strong>
            </div>
            <div class="panel-body">
                <form action="edit_user.php?id=<?php echo (int) $e_user['id']; ?>" method="post" class="clearfix">
                    <div class="form-group">
                        <label for="password" class="control-label"><?php echo PASSWORD; ?></label>
                        <input type="password" class="form-control" name="password" placeholder="<?php echo TYPE_USER_NEW_PASSWORD; ?>">
                    </div>
                    <div class="form-group clearfix">
                        <button type="submit" name="update-pass" class="btn btn-danger pull-right"><? echo CHANGE ?></button>
                    </div>
                </form>
            </div>
        </div>
